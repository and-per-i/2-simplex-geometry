...
Two-pass design:
  KV1 kernel  — computes dK1, dV1 and accumulates dQ via atomic_add
  KV2Q kernel — computes dK2, dV2 and writes dQ (two passes for the KV2 window)

HEAD_DIM_PAD = next_power_of_2(head_dim) — required by Triton for tl.arange/tl.dot;
actual head_dim is passed as a runtime arg and used for load/store masks.

Gradients are accumulated in float32 for numerical stability even when the
forward pass ran in bfloat16.
"""
...
if _check_triton():
    ...
    def two_simplicial_attn_bwd_kv1_kernel(
        ...
        qkv_offs_h = tl.arange(0, HEAD_DIM_PAD)
        ...
        k1_tile = tl.load(K1_ptr + k1_offs, mask=kv1_mask, other=0.0).to(compute_dtype)
        v1_tile = tl.load(V1_ptr + v1_offs, mask=kv1_mask, other=0.0).to(compute_dtype)
        if is_flipped:
            k1_tile += K2_BIAS
            v1_tile += V2_BIAS

        dv1 = tl.zeros((BLOCK_SIZE_KV, HEAD_DIM_PAD), compute_dtype)
        dk1 = tl.zeros((BLOCK_SIZE_KV, HEAD_DIM_PAD), compute_dtype)
        ...
        for kv2_idx in tl.range(tl.maximum(0, kv1_start - w2), tl.minimum(seq_len, kv1_end + w1)):
            ...
            k2_tile = (tl.load(K2_ptr + k2_offs, mask=qkv_mask_h, other=0.0).to(compute_dtype))[None, :]
            v2_tile = (tl.load(V2_ptr + v2_offs, mask=qkv_mask_h, other=0.0).to(compute_dtype))[None, :]
            if not is_flipped:
                k2_tile += K2_BIAS
                v2_tile += V2_BIAS

            k1k2 = (k1_tile * k2_tile).to(gemm_dtype)
            v1v2 = (v1_tile * v2_tile).to(gemm_dtype)

            q_start_inner = tl.maximum(kv1_start, kv2_idx)
            q_end_inner   = tl.minimum(seq_len, tl.minimum(kv1_end + w1, kv2_idx + w2))

            for q_idx in tl.range(q_start_inner, q_end_inner, BLOCK_SIZE_Q):
                ...
                qt_tile = tl.load(Q_ptr + q_offs, mask=qt_mask, other=0.0).to(gemm_dtype)

                m_offs = q_offs_s * m_stride_s
                d_offs = q_offs_s * d_stride_s
                m_tile = tl.load(M_ptr + m_offs, mask=q_mask_s, other=0.0).to(compute_dtype)[None, :]
                d_tile = tl.load(D_ptr + d_offs, mask=q_mask_s, other=0.0).to(compute_dtype)[None, :]

                dO_offs = q_offs_s[:, None] * dO_stride_s + qkv_offs_h[None, :] * dO_stride_h
                dO_tile = tl.load(dO_ptr + dO_offs, mask=q_mask_s[:, None] & qkv_mask_h[None, :], other=0.0).to(compute_dtype)

                if COMPUTE_DQ:
                    dq = tl.zeros((BLOCK_SIZE_Q, HEAD_DIM_PAD), tl.float32)
                ...
                dsT = pT * (dpT - d_tile)
                dsT = tl.where(local_mask, dsT, 0.0).to(gemm_dtype)
    ...
    # ---------------------------------------------------------------------------
    # KV2Q Kernel: dK2, dV2, dQ (two passes)
    # ---------------------------------------------------------------------------
    @triton.autotune(
    ...
    def two_simplicial_attn_bwd_kv2q_kernel(
        ...
        k2_tile  = tl.load(K2_ptr  + kv2_offs, mask=kv2_mask, other=0.0).to(gemm_dtype)
        v2_tile  = tl.load(V2_ptr  + kv2_offs, mask=kv2_mask, other=0.0).to(gemm_dtype)
        m_tile   = tl.load(M_ptr   + q_offs_s * m_stride_s, mask=q_mask_s, other=0.0).to(compute_dtype)
        d_tile   = tl.load(D_ptr   + q_offs_s * d_stride_s, mask=q_mask_s, other=0.0).to(compute_dtype)
        dO_tile  = tl.load(dO_ptr  + dO_offs, mask=q_mask, other=0.0).to(gemm_dtype)

        k2_tile += K2_BIAS
        v2_tile += V2_BIAS

        dq  = tl.zeros((BLOCK_SIZE_KV2 // 2, HEAD_DIM_PAD), tl.float32)
        dk2 = tl.zeros((BLOCK_SIZE_KV2,      HEAD_DIM_PAD), tl.float32)
        dv2 = tl.zeros((BLOCK_SIZE_KV2,      HEAD_DIM_PAD), tl.float32)

        kv1_start_inner = tl.maximum(0, q_start - w1)
        kv1_end_inner   = tl.minimum(seq_len, q_end)
        for kv1_idx in tl.range(kv1_start_inner, kv1_end_inner, num_stages=num_stages):
            k1_offs = kv1_idx * k1_stride_s + qkv_offs_h * k1_stride_h
            v1_offs = kv1_idx * v1_stride_s + qkv_offs_h * v1_stride_h
            k1_tile = tl.load(K1_ptr + k1_offs, mask=qkv_mask_h, other=0.0).to(compute_dtype)
            v1_tile = tl.load(V1_ptr + v1_offs, mask=qkv_mask_h, other=0.0).to(compute_dtype)

            qk1_s = (q_tile * (k1_tile[None, :] * softmax_scale)).to(gemm_dtype)
            ...
            qkT_mask    = (kv2_mask_s[:, None] & q_mask_s[None, :]) & local_mask
            qkkT = tl.where(local_mask, qkkT, -1.0e38)
            ...
            dq  += tl.dot(dsT.T, k1k2, out_dtype=tl.float32)

        if IS_SECOND_PASS:
            prev_dk2 = tl.load(dK2_ptr + kv2_offs, mask=kv2_mask, other=0.0)
            prev_dv2 = tl.load(dV2_ptr + kv2_offs, mask=kv2_mask, other=0.0)
            dk2 += prev_dk2
            dv2 += prev_dv2

        dq *= softmax_scale
        tl.store(dK2_ptr + kv2_offs, dk2.to(data_dtype), mask=kv2_mask)
        tl.store(dV2_ptr + kv2_offs, dv2.to(data_dtype), mask=kv2_mask)
        tl.store(dQ_ptr  + q_offs,   dq.to(data_dtype),  mask=q_mask)
...
def backward(grad_output, x, Q, K, V, Kp, Vp, O, M, out_dim, num_heads, head_dim, w1=8, w2=8):
    ...
    if Q.dim() == 3:
        ...
        dO_r, O_r, M_r = grad_output.contiguous(), O.contiguous(), M.contiguous()

    # D_row = rowsum(dO * O): [B, H, S]
    D_row = (dO_r.float() * O_r.float()).sum(dim=3).permute(0, 2, 1).contiguous()

    # Gradients in float32 for stability
    dQ  = torch.zeros_like(Q_r,  dtype=torch.float32)
    dK1 = torch.zeros_like(K_r,  dtype=torch.float32)
    dV1 = torch.zeros_like(V_r,  dtype=torch.float32)
    dK2 = torch.zeros_like(Kp_r, dtype=torch.float32)
    dV2 = torch.zeros_like(Vp_r, dtype=torch.float32)

    q_s, k1_s, k2_s, v1_s, v2_s = Q_r.stride(), K_r.stride(), Kp_r.stride(), V_r.stride(), Vp_r.stride()
    do_s, m_s, d_s, dq_s = dO_r.stride(), M_r.stride(), D_row.stride(), dQ.stride()
    ...
    two_simplicial_attn_bwd_kv1_kernel[grid_kv1](
        ...
        **strides_kv1,
        ...
        HEAD_DIM_PAD=head_dim_pad, SM_SCALE=1.0 / (head_dim ** 0.5),
    ...
    for is_second_pass in (False, True):
        two_simplicial_attn_bwd_kv2q_kernel[grid_kv2q](
            ...
            **strides_kv2,
            HEAD_DIM_PAD=head_dim_pad, SM_SCALE=1.0 / (head_dim ** 0.5),
            K2_BIAS=0.0, V2_BIAS=0.0, IS_SECOND_PASS=is_second_pass,
        )

    if Q.dim() == 3:
        return (
            dQ.view(seq_len, num_heads, head_dim),
            dK1.view(seq_len, num_heads, head_dim),
            dV1.view(seq_len, num_heads, head_dim),
            dK2.view(seq_len, num_heads, head_dim),
            dV2.view(seq_len, num_heads, head_dim),
        )
